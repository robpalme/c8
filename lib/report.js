...
const { isAbsolute, resolve, extname, join, dirname, sep } = require('path')
...
class Report {
  ...
  _normalizeProcessCov (v8ProcessCov, fileIndex) {
    ...
    for (const v8ScriptCov of v8ProcessCov.result) {
      ...
      const sourcePath = this._decodePath(v8ScriptCov.url)

      // Record that empty-reportgeneration is not needed for either file
      fileIndex.add(v8ScriptCov.url)
      fileIndex.add(sourcePath)

      if (this.exclude.shouldInstrument(sourcePath) &&
  ...
  /**
   * Remaps an absolute executable path to an absolute source path.
   *
   * Returns the original path if there is nothing to decode.
   *
   * @param path An absolute executable script path.
   * @return {string} An absolute source script path.
   * @private
   */
  _decodePath (path) {
    if (!isAbsolute(path) || path.split(sep).includes('node_modules')) {
      return path
    }

    const sourceMapPath = getSourceMapFromFile(path)
    if (!sourceMapPath) {
      return path
    }
    ...
    return isAbsolute(candidatePath)
      ? candidatePath
      : resolve(dirname(path), candidatePath)
  }
}
